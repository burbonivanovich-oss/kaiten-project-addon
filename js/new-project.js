...
/* Про раскладку досок — на будущее, если досок снова станет больше одной.
 *
 * У доски есть координаты top/left. Новая доска по умолчанию получает
 * left = 0 и встаёт ПОД предыдущей, а не рядом — пространство открывается
 * «лесенкой», и вторая доска уезжает за первый экран. Чтобы поставить в одну
 * линию, второй доске нужен left, равный ширине первой (одноколоночная — 304).
 *
 * Сейчас доска одна, и вопрос не стоит: карточке некуда упасть мимо.
 */
...
async function init() {
  ...
  document.getElementById('f').addEventListener('submit', async (ev) => {
    ...
    try {
      ...
      /* Одна доска, а не две.
       *
       * Раньше шаблон заводил ещё «Ключевое о проекте» — доску ради одной
       * справочной карточки. Две беды сразу:
       *
       * 1. Она вставала первой в пространстве, и нативное «Создать карточку»
       *    целилось в НЕЁ. Задача уезжала в справку вместо рабочей доски —
       *    молча, потому что «Расположение» в диалоге уже заполнено.
       * 2. Её содержимое дублировало описание карточки проекта: цель, метрика,
       *    срок и связанная цель были записаны в обоих местах, и расходились.
       *
       * Контекст проекта живёт в описании карточки проекта — там его и ищут
       * машинист с куратором. Памятка «три действия» одинакова для всех
       * проектов и должна лежать один раз у команд, а не копией в каждом
       * пространстве. */
      msg('Создаю доску задач…');
